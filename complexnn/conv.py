...
## Complex-valued convolution with concatenated output
class ComplexConvConcat(Layer):
    """Abstract nD complex convolution layer.
    This layer creates a complex convolution kernel that is convolved
    with the layer input to produce a tensor of outputs.
    If `use_bias` is True, a bias vector is created and added to the outputs.
    ...
    def __init__(self, rank,
                 ...
                 epsilon=1e-7,
                 **kwargs):
        super(ComplexConvConcat, self).__init__(**kwargs)
        self.rank = rank
        self.filters = filters
        self.kernel_size = conv_utils.normalize_tuple(kernel_size, rank, 'kernel_size')
        self.strides = conv_utils.normalize_tuple(strides, rank, 'strides')
    ...
    def call(self, inputs):
        ...
        input_re, input_im = tf.split(inputs, num_or_size_splits=2, axis=1)

        output_re_re = convFunc(input_re, f_real, **convArgs)
        output_re_im = convFunc(input_re, f_imag, **convArgs)
        output_im_re = convFunc(input_im, f_real, **convArgs)
        output_im_im = convFunc(input_im, f_imag, **convArgs)

        output_re = K.concatenate([output_re_re, output_im_im], axis=1)
        output_im = K.concatenate([output_re_im, output_im_re], axis=1)

        output = K.concatenate([output_re, output_im], axis=1)
    ...
    def get_config(self):
        ...
        base_config = super(ComplexConvConcat, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))

class ComplexConvConcat2D(ComplexConvConcat):
    """2D Complex convolution layer (e.g. spatial convolution over images).
    This layer creates a complex convolution kernel that is convolved
    with a complex input layer to produce a complex output tensor. If `use_bias`
    is True, a complex bias vector is created and added to the outputs.
    ...
    def __init__(self, filters,
                 ...
                 spectral_parametrization=False,
                 **kwargs):
        super(ComplexConvConcat2D, self).__init__(
            rank=2,
            filters=filters,
            kernel_size=kernel_size,
            strides=strides,
    ...
    def get_config(self):
        config = super(ComplexConvConcat2D, self).get_config()
        config.pop('rank')
        return config